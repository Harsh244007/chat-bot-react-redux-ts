...
export const generateMessage = (date: Date, time: string) => {
  ...
  const formattedEndHour =
    parseInt(time.split("-")[1]) >= 13
      ? parseInt(time.split("-")[1]) - 12
      : parseInt(time.split("-")[1]);
  const amPm = parseInt(time.split("-")[0]) >= 13 ? "PM" : "AM";

// @ts-ignore
  const message = createClientMessage(`${getDate} ${months[getMonth]}, ${getDay === 0 ? days[6] : days[getDay - 1]}DAY ${formattedStartHour} - ${formattedEndHour} ${amPm}`);
